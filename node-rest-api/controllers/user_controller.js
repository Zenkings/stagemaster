...
const getActiveUsersCount = (req, res, next) => {
  pool.query("SELECT COUNT(*) AS active_users_count FROM reactjs.users", [], function(err, result) {
    if (err) {
      res.status(400).send(err);
    }
    res.status(200).json({ activeUsersCount: result.rows[0].active_users_count });
  });
};
...
const VerificationOperateurSecuriter = (req,res,next)=>{
 ...
 module.exports = {getUsers,getActiveUsersCount,insertUsers,getLog,deleteUser,getInfoLog,UpdateUser,getUserFromGpao,VerificationOperateurSecuriter,getNb_echec};
