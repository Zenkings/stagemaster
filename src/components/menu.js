...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    if (cookies.islogged_react === "true") {
      // Si l'utilisateur est connecté, rétracte le menu par défaut
      setMenuCollapse(true);
    } else {
      // Sinon, laisse le menu étendu par défaut
  ...
  const CollapseMenu = () => {
    ...
    if (cookies.islogged_react === "true") {
      // Si l'utilisateur est connecté, rétracte le menu
      setMenuCollapse(!MenuCollapse);
    } else {
      // Sinon, permet à l'utilisateur de basculer le menu librement
      setMenuCollapse(!MenuCollapse);
    }
  };
      ...
      <div
        className={
          !MenuCollapse
            ...
            : background + " fixed-top text-center  "
        }
        style={!MenuCollapse ? styleMenuDefault : styleMenuMinim}>
        <div className="menus mt-1">
          <div className="title text-white text-center">
            <h6 className={!MenuCollapse ? " fs-4 titleMenu " : "d-none"}>
              React App
            </h6>
            <button
              className={
              ...
              <i
                className={
                  !MenuCollapse
                    ? "bi bi-arrow-left-circle text-warning"
                    : "bi bi-arrow-right-circle text-warning fs-5 "
                }></i>{" "}
            </button>
            <h5 className={!MenuCollapse ? "titreProfil" : "d-none"}>
              {matricule}{" "}
            </h5>
          </div>
          ...
          <ul className="nav d-block mt-3 scroller">
              ...
              .map((menu, index) =>
                menu != undefined ? (
                  menu.route == "" && menu.sous_menus.length != 0 ? (
                    <li
                      className="nav-item nav-sousmenu"
                      data-bs-toggle="tooltip"
                      data-bs-placement="right"
                      title={menu.labelle_menu}
                      key={index}>
                      <NavLink
                        className="btn btn-dark nav-link lien rounded-1 text-white d-flex justify-content-between"
                        to="Test"
                        data-bs-toggle="collapse"
                        data-bs-target={"#collapseMenu" + index}
                        aria-expanded="false"
                        ...
                        <div
                          className={
                            !MenuCollapse
                              ...
                              : background + " card p-0 pt-1 text-center "
                          }>
                          {menu.sous_menus
                            .filter((sous_menus) =>
                              sous_menus.role_sous_menu.includes(role)
                            )
                            .map((sous_menus, index) => (
                              <NavLink
                                className="nav-link  lien rounded-1 mb-2 d-flex justify-content-between"
                                to={"react_app/" + sous_menus.route_sous_menu}
                                style={({ isActive }) => ({
                                  background: isActive ? "#00000078" : "",
                                  color: isActive ? "#7ee2a6" : "#ffffff",
                                })}>
                                {!MenuCollapse ? (
                                  <span className="titleMenu ms-2 d-inline">
                                    {" "}
                                    {sous_menus.nom_sous_menu}
                                  </span>
                                ) : (
                                  <></>
                                )}
                    ...
                    <li
                      ...
                      key={index}>
                      <NavLink
                        className="nav-link lien rounded-1 d-flex justify-content-between"
                        style={({ isActive }) => ({
                          background: isActive ? "#00000078" : "",
                          color: isActive ? "#7ee2a6" : "#ffffff",
                        })}
                        to={"react_app/" + menu.route}>
                        {!MenuCollapse ? (
                          <span className="titleMenu d-inline">
                            {menu.labelle_menu}
                          </span>
                        ) : (
                          <></>
            ...
            <li className="nav-item nav-sousmenu ">
              <button
                className="btn btn-dark nav-link lien rounded-1 w-100 text-white d-flex justify-content-between"
                data-bs-toggle="collapse"
                data-bs-target="#theme"
                aria-expanded="false"
                aria-controls="collapseExample">
                {!MenuCollapse ? (
                  <span className="titleMenu"> Thèmes</span>
                ) : (
                  <></>
                )}
              ...
              <div className="collapse" id="theme">
                <div
                  ...
                  <button
                    className="btn btn-dark lien rounded-1 mb-2 text-center"
                    onClick={darkMode}>
                    <i
                      className={
                        theme ? "bi bi-brightness-high" : "bi bi-moon"
                      }></i>
                    {!MenuCollapse ? (
                      <span className="titleMenu ms-2">
        ...
        <div className="log">
          <button
            className="btn  btn-sm form-control text-white"
            onClick={handleLogout}>
            <i className="bi bi-box-arrow-right"></i>{" "}
            <span
              className={
                !MenuCollapse ? "logoutText text-uppercase" : "d-none"
              }>
              Se déconnecter
            </span>
          </button>
